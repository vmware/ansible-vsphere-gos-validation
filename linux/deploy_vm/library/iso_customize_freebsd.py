...
def iso_rebuild(module, src_iso, dest_iso, delete_files_list, add_files_list):
    ...
    try:
        ...
        # Forcefully override the allow_duplicate behavior and silence duplicate name exceptions
        orig_add_child = pycdlib.dr.DirectoryRecord._add_child
        def tolerant_add_child(self, child, logical_block_size, allow_duplicate, *args, **kwargs):
            try:
                return orig_add_child(self, child, logical_block_size, True, *args, **kwargs)
            except pycdlib.pycdlibexception.PyCdlibInvalidInput as e:
                if 'duplicate name' in str(e):
                    return
                raise e
        pycdlib.dr.DirectoryRecord._add_child = tolerant_add_child
